...
#Ore selection data
var ores: Array = []
@onready var ore_finder = $OreFinder
...
func _process(delta):
	...
	if d>2*PI:
		d = 0
	position = (Vector2(sin(d*orbit_speed)*radius,cos(d*orbit_speed)*radius)+orbit_target.global_position)
...
func _on_enemy_finder_body_exited(body):
	ores.erase(body)
	if ores.size()==0 and not is_instance_valid(shooting_target):
		shoot_timer.stop()
